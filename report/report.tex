...
Our system uses Raft-based primary backup replication to implement a strongly consistent metadata stire.
The approach is appropriate because Raft provides a well-specified, leader-driven consensus protocol that is easier
...
Raft's correctness is due to the following invariants:
...
Raft's simplicity allowed us to write a clean, channel-driven state machine in Go, where all the consensus dependant transitions occue in a
single gorouting (\texttt{doFollower}, \texttt{doCandidate}, \texttt{doLeader}).
...
Our system tolerates crash failures of leaders and followers by a combination of mechanisms.
One such way is having followers promote themselves when their election timeout fires. Another one is forcing leaders and candidates
to step down immediately on observing higher-term RPCs. Finally, we ensuring committed entries survive leader crashes via quorum replication
and the log-matching rule.

Our Test cases validate:
\begin{itemize}
    \item \textbf{Leader crash before commit} -> entry never becomes visible. (Test 6).
    \item \textbf{Leader crash after commit} -> new leader preserves committed state (Test 7).
\end{itemize}

\subsubsection{Performance}

We have several design choices that optimize low-latenct and high throughput. We use asynchronous and pipleines replication by having
Leaders send AppendEntries in parallel to all followers. Another intentional design choice is having only one gorouting 
mutate the Raft state and the RPC handlers defer to channels, which reduces contention. We also implement incremental commit by having
the leader commit entries as soon as a majority acknowledges, avoiding batching delays. Finally, we use client-server streaming so that clients maintain a bidirectional streaming session for low-latency updates.
