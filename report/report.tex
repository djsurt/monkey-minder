\documentclass[journal,twocolumn,american]{IEEEtran}
\usepackage[T1]{fontenc}
\usepackage{lmodern}
\usepackage[utf8]{inputenc}
\usepackage[english]{babel}
\usepackage[pdfusetitle,
 bookmarks=true,bookmarksnumbered=false,bookmarksopen=false,
 breaklinks=false,pdfborder={0 0 1},backref=false,colorlinks=false]
 {hyperref}
\usepackage{lipsum}
\usepackage{graphicx}
\usepackage{calc}
\usepackage[style=ieee]{biblatex}
\usepackage{amsmath}

\renewcommand{\ttdefault}{cmtt}

\addbibresource{sources.bib}

\ifx\mmCfgTodos\undefined
    \usepackage[disable]{todonotes}
\else
    \usepackage{todonotes}
    \newcommand{\tinierForTODOs}{\fontsize{6}{0}\selectfont}
    \setuptodonotes{size=\tinierForTODOs,noinlinepar,inlinewidth=2in,tickmarkheight=0.1cm}
\fi

% via https://stackoverflow.com/a/123184/8762161
\newlength{\mmdfigureWidthTmp}

% TODO(amgg): should just make mmdfigure call this
\newcommand{\easyfigure}[2]{\begin{figure}[h]%
    % get the width of the figure unscaled %
    \settowidth{\mmdfigureWidthTmp}{\includegraphics{#1}}%
    % give the diagrams a constant scale down regardless of their size. %
    % (based on a rough conversion between their font sizes, assuming i'm correct about those. -amgg) %
    \setlength{\mmdfigureWidthTmp}{\mmdfigureWidthTmp/100*10/16*100}%
    % scale that down further but only if necissary to not exceed text width %
    % (note that we use columnwidth here, not textwidth, since we're in a 2-column layout) %
    \setlength{\mmdfigureWidthTmp}{\minof{\mmdfigureWidthTmp}{\columnwidth}}%
    \centering%
    \includegraphics[width=\mmdfigureWidthTmp]{#1}%
    \caption{#2}%
\end{figure}%
}

\newcommand{\mmdfigure}[2]{\begin{figure}[h]%
    % get the width of the figure unscaled %
    \settowidth{\mmdfigureWidthTmp}{\includegraphics{#1.mmd.pdf}}%
    % give the diagrams a constant scale down regardless of their size. %
    % (based on a rough conversion between their font sizes, assuming i'm correct about those. -amgg) %
    \setlength{\mmdfigureWidthTmp}{\mmdfigureWidthTmp*10/16}%
    % scale that down further but only if necissary to not exceed text width %
    % (note that we use columnwidth here, not textwidth, since we're in a 2-column layout) %
    \setlength{\mmdfigureWidthTmp}{\minof{\mmdfigureWidthTmp}{\columnwidth}}%
    \centering%
    \includegraphics[width=\mmdfigureWidthTmp]{#1.mmd.pdf}%
    \caption{#2}%
\end{figure}%
}

\newcommand{\mmdfiguretodo}[2]{\begin{figure}[h]%
    \centering%
    \missingfigure{#1}%
    \caption{#2}%
\end{figure}%
}

\ifx\mmCfgNofigures\undefined
\else
    \renewcommand{\mmdfigure}[2]{}
    \renewcommand{\mmdfiguretodo}[2]{}
\fi

% for key words/phrases
\newcommand{\term}[1]{\textit{#1}}
% for references to class/function names
\newcommand{\codeterm}[1]{\texttt{#1}}

\newcommand{\todocritical}[2][]{\todo[color=red, caption={CRITICAL: #2}, #1]{#2}}
\newcommand{\todonitpick}[2][]{\todo[color=cyan, caption={nitpick: #2}, #1]{#2}}


\AtEveryBibitem{%
    \ifentrytype{online}{}{
        \clearfield{url}
        \clearfield{urldate}
        \clearfield{urlyear}
        \clearfield{urlmonth}
        \clearfield{urlday}
    }
}


\begin{document}

% title stuff
\title{Monkey Minder}
\author{%
Caleb~Fringer~(016632603,~caleb.fringer@sjsu.edu)\\%
Adrian~Guerra~(013938652,~adrian.guerra@sjsu.edu)\\%
Dhananjay~Surti~(019111482,~dhananjay.surti@sjsu.edu)%
}
\maketitle

\listoftodos

\section{Goals and Motivation}
\input{sections/goals.tex}

\section{Challenges Addressed}
\input{sections/challenges.tex}

\section{Prior Art}
\input{sections/prior_art.tex}

\section{Design}
\input{sections/design.tex}

\section{Evaluation}
\input{sections/evaluation.tex}

\section{Implementation Details}
\input{sections/implementation.tex}

\section{Demonstration}
\input{sections/demonstration.tex}

\section{Performance}
\input{sections/performance.tex}

\section{Testing}
\input{sections/testing.tex}

\section{Conclusions}
\input{sections/conclusions.tex}

\printbibliography[title=References]

\end{document}

