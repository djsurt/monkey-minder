...
An important challenge is \textbf{consistency}. In a distributed environment with concurrent clients, and asynchronous
communication, ensuring that all nodes observe a coherent view of the shared state is non-trivial.
Without careful coordination, concurrent updates could lead to divergent replicas, fractured writes, or stale reads.
This challenege is particularly evident in a system like ours, where inconsistencies in even small subtrees can 
propagate upwards and corrupt the global namespace.

A related challenge is that of \textbf{fault tolerance}. Nodes may crash, restart with stale state, or 
be unable to communicate due to partitions. The system must continue making progress despite these failures
and must guarantee that committed updates are never lost or partially applied. Acheiving fault tolerance while preserving strong consistency
imposes strict requirements on how nodes coordinate, elect leaders, and recover from failures.

\textbf{Concurrency} is also an essential challenge. Distributed systems process many requests simultaneously, each
potentially intercting with shared adat or competing for state. Without a proper ordering or serialization mechanism,
concurrent operations could produce non-deterministic outcomes. This is especially relevant for operations like create, update,
and delete, which must appear atomic to clients.

\textbf{Scalability} is an essential challenge to be considered. As the number of clients and nodes grows, the system must maintain low latency
and high throughput. A Zookeeper-like service is read-heavy and the design requires a design that can handle high read concurrency
without overloading a single node while also maintaining strong consistency guarantees.

In addition, \textbf{transparency} is very important for a usable distributed system. Clients should not
need to know which node is the current leader, whether a failover has occured, or how the system internally
coordinates. Masking operational details simplifies the client interface and reduces risk of incorrect client behavior.

\textbf{Security} is absolutely necessary in a distributed system. Communication between nodes and clients should not travel
though untrusted networks as this raises concerns about eavesdropping, forged messages, and unauthorized access.

Finally, \textbf{heterogeneity and openness} are important considerations in a distributed system. A practical system
should support clients written in different languages, and operate across different deployment environments. These
requirements influence protocol choices and and structures of the communication layer.
...
TODO: Need to finish this section with the diagrams
...
% \mmdfigure{usecase-overview}{System use-case overview.\label{fig:usecase}}
...
% \mmdfigure{deployment}{Cluster deployment and component diagram.\label{fig:deployment}}
...
Our watch system is implemented by the \texttt{WatchManager} type, which tracks a slice of pending watches guarded 
by a mutex. Each watch consists of a predicate over committed log entries and a completion channel that is signaled exactly
once.

When a leader commits a new log entry, it calls \texttt{wacthes.SubmitEntry(entry, index)}. The watch manager acquires the mutex
and uses \texttt{slices.DeleteFunc} to remove all watches whose predicate returns true for the given entry and indexm sending a notification 
on the corresponding channel. Watches that do not match remain in the list and will eventually be evaluated against future commits.
This ensures tht notifications are triggered strictly on committed history and that each watch is fired at most once.
