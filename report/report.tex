...
Our system tolerates crash failures of leaders and followers by:
\begin{itemize}
    \item Having followers promote themselves when their election timeout fires.
    \item Forcing leaders and candidates to step down immediately on observing higher-term RPCs.
    \item Ensuring committed entries survive leader crashes via quorum replication and the log-matching rule. 
\end{itemize}
...
\begin{itemize}
    \item \textbf{Asynchronous, pipelined replication}: leaders and send AppendEntries in parallel to all followers.
  \item \textbf{Low contention}: only one goroutine mutates Raft state; RPC handlers defer to channels.
  \item \textbf{Incremental commit}: the leader commits entries as soon as a majority acknowledges, avoiding batching delays.
  \item \textbf{Client–server streaming}: clients maintain a bidirectional streaming session for low-latency updates.
\end{itemize}
