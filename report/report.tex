...
Appending an entry involves applying the entry to the tail snapshot, storing it in the log, and updating any existing checkpoints.
Retrival is supported using an index-based lookup like \texttt{GetEntryAt} and \texttt{GetEntryLatest}, both of which are used
by Raft for log-matching during leader election and AppendEntries processing. The log also provides a compaction mechanism:
\texttt{SquashFirstN} and \texttt{SquashUntil} merge older entries into the head snapshot once they are no longer needed, preventing
unbounded growth.
